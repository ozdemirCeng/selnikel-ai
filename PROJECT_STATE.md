...
| **Evaluation Benchmark** | `RAGBenchmarkEvaluator` + `app.cli.benchmark_runner` | 28 versioned questions across 8 categories, multi-mode runner (self-check, pure retrieval BM25, full-rag live/skipped), immutable JSON reports, RBAC | **OPERATIONAL (STAGE P1.2 VERIFIED)** | Full test suite passed (127 passed, 1 skipped). `test_golden_benchmark_suite.py` (10/10 passed), `test_parser_table_fidelity.py` (7/7 passed). |
